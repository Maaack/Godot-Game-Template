...
This effectively deletes the current `addons/maaacks_ui_sound_controller/` folder and replaces it with a new one. Nothing outside of `addons/` should be affected.
...
To disable automatic updates by plugin, go into the **Project > Project Settings > General** tab, and remove the entries in the `plugin_updater/plugins` setting (ex. `"res://addons/maaacks_ui_sound_controller/"`). You can then close the window.
